...
expected worst-case space complexity is O(X), beyond input storage (not counting 
the storage required for input arguments).
Elements of input arrays can be modified.*/

package lesson2_countingElement.frogRiverOne;
...
class Solution {
 ...
 public int solution(int X, int[] A) {
     ...
     int count=0;
     int i=0;
     for(;i<A.length&&count<X;i++){
         if(A[i]<=X && B[A[i]-1]==0 && count<X){
             B[A[i]-1]++;
             count++;
         }
     }
     return count<X?-1:i-1;
 }
}